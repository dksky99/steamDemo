                                   ...
                                   TFunction<void(FName, bool)> Callback)
{
	...
	LastSessionSettings->Set(SETTING_MAPNAME, FString(TEXT("some level")), EOnlineDataAdvertisementType::ViaOnlineServiceAndPing);

	// the custom name for the session, provided by the user
	LastSessionSettings->Set(SETTING_CUSTOMNAME, CustomName, EOnlineDataAdvertisementType::ViaOnlineServiceAndPing);
	...
	CDisplayLog::Log(TEXT("%s Setting_Level : %d"), *GetFullName(),(int32)ECurrentLevel::SomeLevel);
	...
	if(SI->OnCreateSessionCompleteDelegates.IsBound())
	{
		...
		UE_LOG(LogNet, Warning, TEXT("%s: OnCreateSessionCompleteDelegates: was bound, clearing"), *GetFullName());
			CDisplayLog::Log(TEXT("%s: OnCreateSessionCompleteDelegates: was bound, clearing"), *GetFullName());
...
{
	const IOnlineSessionPtr SI = GetSessionInterface();
	const TSharedPtr<FOnlineSessionSearch> LastSessionSearch = MakeShareable(new FOnlineSessionSearch());
	...
	CDisplayLog::Log(TEXT("Try JoinSession %s "), *GetFullName());
	...
	if(SI->OnFindSessionsCompleteDelegates.IsBound())
	{
		...
		CDisplayLog::Log(TEXT("%s: OnFindSessionsCompleteDelegates: was bound, clearing"), *GetFullName());
	...
	SI->OnFindSessionsCompleteDelegates.AddLambda([this, LastSessionSearch, LPC, Callback, SI] (bool bSuccess)
	{
		...
		CDisplayLog::Log(TEXT("Find Session Success : %s "), bSuccess?TEXT("True"): TEXT("False"));
		...
		if(bSuccess && !Results.IsEmpty())
		{
			if(SI->OnJoinSessionCompleteDelegates.IsBound())
			{
				...
				CDisplayLog::Log(TEXT("%s: OnJoinSessionCompleteDelegates: was bound, clearing"), *GetFullName());
			...
			for (auto result : Results)
			{
				...
				FString MapName;
				result.Session.SessionSettings.Get(SETTING_CUSTOMNAME, MapName);
				CDisplayLog::Log(TEXT("Session Map Name : %s "),*MapName );
				CDisplayLog::Log(TEXT("Session Map Name : %s "),*MapName );
				if (MapName == "LDKTesting")
				{
					CDisplayLog::Log(TEXT("Success Find Map  : %s "), *MapName);
					result.Session.SessionSettings.Get(SETTING_LEVEL, NewLevelI);
					SI->OnJoinSessionCompleteDelegates.AddLambda([Callback, NewLevelI](FName, EOnJoinSessionCompleteResult::Type Type)
						{
							CDisplayLog::Log(TEXT("Session Map Level : %d "), NewLevelI);

							// to convert `int32` to the enum, `static_cast` is just fine
							Callback(static_cast<ECurrentLevel>(NewLevelI), Type);
							//Callback(static_cast<ECurrentLevel>(1), Type);
						});

					CDisplayLog::Log(TEXT("Try Join Map  : %s "), *MapName);
					SI->JoinSession(LPC.GetLocalPlayer()->GetIndexInGameInstance(), NAME_GameSession, result);
		...
		else
		{
			...
			CDisplayLog::Log(TEXT("%s: couldn't find session."), *GetFullName());
		}
	});
...
void UMyGISubsystem::LeaveSession()
{
	...
	if(SI->GetNamedSession(NAME_GameSession))
	{
		if(SI->OnDestroySessionCompleteDelegates.IsBound())
		{
			...
			CDisplayLog::Log(TEXT("%s: OnDestroySessionCompleteDelegates: was bound, clearing"), *GetFullName());
		...
		SI->OnDestroySessionCompleteDelegates.AddLambda([this, SI] (FName, bool bSuccess)
		{
			...
			if(SI->GetNamedSession(NAME_GameSession))
			{
				...
				CDisplayLog::Log(TEXT("%s: Failed to destroy session, trying again ..."), *GetFullName());
				SI->DestroySession(NAME_GameSession);
			}
			else
			{
				UE_LOG(LogTemp, Warning, TEXT("%s: Session destroyed."), *GetFullName());
				CDisplayLog::Log(TEXT("%s: Session destroyed."), *GetFullName());
...
void UMyGISubsystem::ShowLoginScreen(const FLocalPlayerContext& LPC)
{
	...
	const IOnlineIdentityPtr OSSIdentity = Online::GetIdentityInterfaceChecked(FName(TEXT("Steam")));
...
void UMyGISubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	UE_LOG(LogNet, Error, TEXT("%s: Try Login"), *GetFullName());
	CDisplayLog::Log(TEXT("%s: Try Login"), *GetFullName());
	/*
	 * `GetIdentityInterfaceChecked(FName(TEXT("EOS"))` causes a crash when EOS isn't configured
	 */
	const IOnlineIdentityPtr OSSIdentity = Online::GetIdentityInterface(GetWorld(), FName(TEXT("Steam")));
	if(!OSSIdentity.IsValid())
	{
		UE_LOG
			( LogNet
			, Warning
			, TEXT("%s: couldn't get identity interface for Steam. Probably not configured.")
			, *GetFullName()
			);
		CDisplayLog::Log(TEXT("%s: couldn't get identity interface for Steam. Probably not configured."), *GetFullName());
		return;
	}
	...
	GEngine->OnNetworkFailure().AddLambda([this] (UWorld*, UNetDriver*, ENetworkFailure::Type, const FString& StrError)
	{
		...
		CDisplayLog::Log(TEXT("%s: NetworkFailure: %s"), *GetFullName(), *StrError);
		LeaveSession();
	});
	...
	if(OSSIdentity->OnLoginCompleteDelegates->IsBound())
	{
		...
		CDisplayLog::Log(TEXT("%s: OnLoginCompleteDelegates: was bound, clearing"), *GetFullName());
	...
	OSSIdentity->OnLoginCompleteDelegates->AddLambda([this] (int32 LocalUserNum, bool bSuccess, const FUniqueNetId& NewUNI, const FString& Error)
	{
		...
		CDisplayLog::Log(TEXT("%s: Login of player num %d: %s"), *GetFullName(), LocalUserNum, bSuccess ? TEXT("success") : TEXT("failure"));
	...
	if(OSSIdentity->OnLogoutCompleteDelegates->IsBound())
	{
		...
		CDisplayLog::Log(TEXT("%s: OnLogoutCompleteDelegates: was bound, clearing"), *GetFullName());
	...
	OSSIdentity->OnLogoutCompleteDelegates->AddLambda([this] (int32 PlayerNum, bool bSuccess)
	{
		...
		CDisplayLog::Log(TEXT("%s: Player %d log out: %s"), *GetFullName(), PlayerNum, bSuccess ? TEXT("success") : TEXT("failure"));
	});
}
