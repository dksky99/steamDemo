	...
	GISub->CreateSession
		...
		, [this, LPC] (FName SessionName, bool bSuccess)
		{
			if(bSuccess)
			{
				...
				CDisplayLog::Log(TEXT("%s: Try ServerTravel"), *GetFullName());
			...
			else
			{
				...
				CDisplayLog::Log(TEXT("%s: create session call returned with failure"), *GetFullName());
...
void UMyGameInstance::JoinGame(const FLocalPlayerContext& LPC)
{
	...
	GetSubsystem<UMyGISubsystem>()->JoinSession(LPC, [this, LPC] (ECurrentLevel NewLevel, EOnJoinSessionCompleteResult::Type Result)
	{
			...
			UE_LOG(LogNet, Error, TEXT("%s: Join session Fininsh"), *GetFullName());
			CDisplayLog::Log(TEXT("%s: Join session Fininsh "), *GetFullName());
		switch(Result)
		{
		using namespace EOnJoinSessionCompleteResult;
		case SessionIsFull:
			UE_LOG(LogNet, Error, TEXT("%s: Join session: session is full"), *GetFullName());
				CDisplayLog::Log(TEXT("%s: Join session: session is full"), *GetFullName());
			break;
		case SessionDoesNotExist:
			UE_LOG(LogNet, Error, TEXT("%s: Join session: session does not exist"), *GetFullName());
			CDisplayLog::Log(TEXT("%s: Join session: session does not exist"), *GetFullName());
			break;
		case CouldNotRetrieveAddress:
			UE_LOG(LogNet, Error, TEXT("%s: Join session: could not retrieve address"), *GetFullName());
			CDisplayLog::Log(TEXT("%s: Join session: could not retrieve address"), *GetFullName());
			break;
		case AlreadyInSession:
			UE_LOG(LogNet, Error, TEXT("%s: Join session: alreayd in session"), *GetFullName());
			CDisplayLog::Log(TEXT("%s: Join session: alreayd in session"), *GetFullName());
			break;
		case UnknownError:
			UE_LOG(LogNet, Error, TEXT("%s: Join session: unknown error"), *GetFullName());
			CDisplayLog::Log(TEXT("%s: Join session: unknown error"), *GetFullName());
			break;
		case Success:
			UE_LOG(LogNet, Error, TEXT("%s: Join session Success"), *GetFullName());
			CDisplayLog::Log(TEXT("%s: Join session Success"), *GetFullName());
			...
			if(ClientTravelToSession(LPC.GetLocalPlayer()->GetControllerId(), NAME_GameSession))
			{
				...
				UE_LOG(LogNet, Error, TEXT("%s: travel to session Level %d"), *GetFullName(), NewLevel);
				CDisplayLog::Log(TEXT("%s: travel to session Level %d"), *GetFullName(), NewLevel);

				UE_LOG(LogNet, Error, TEXT("%s: travel to session Success %d"), *GetFullName(), Cast<UMyLocalPlayer>(LPC.GetLocalPlayer())->CurrentLevel);
				CDisplayLog::Log(TEXT("%s: travel to session Success %d"), *GetFullName(), Cast<UMyLocalPlayer>(LPC.GetLocalPlayer())->CurrentLevel);
			}
			else
			{
				UE_LOG(LogNet, Error, TEXT("%s: travel to session failed"), *GetFullName());
				CDisplayLog::Log(TEXT("%s: travel to session failed"), *GetFullName());
			}
			break;
		default: ;
		}
